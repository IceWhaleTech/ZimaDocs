...
> **协议范围：** 本页重点介绍单个 Docker 应用的适配。当前 v2 商店字段、locale 大小写、仓库结构和构建产物请以 [Docker Compose 与 x-casaos](./app-store-compose-x-casaos)、[仓库结构](./app-store-repository-structure) 和 [构建产物](./app-store-build-output) 为准。
...
> 此处保留精简示例用于说明应用适配流程。完整的 v2 必填字段和允许的分类请以 [Docker Compose 与 x-casaos](./app-store-compose-x-casaos) 为准。

```language
x-casaos:
    id: org.syncthing.syncthing       # 稳定的反向域名应用 ID
    architectures:                  # 支持的架构列表
        - amd64
        - arm
        - arm64
    version: "1.0.0"                # v2 商店展示的应用版本
