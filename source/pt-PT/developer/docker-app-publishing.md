...
> **Âmbito do protocolo:** Esta página concentra-se na adaptação de uma aplicação Docker individual. Para o esquema v2 atual, a capitalização das chaves locale, a estrutura do repositório e os resultados da compilação, consulte [Docker Compose e x-casaos](./app-store-compose-x-casaos), [Estrutura do repositório](./app-store-repository-structure) e [Resultados da compilação](./app-store-build-output).
...
### Exemplo legado de x-casaos

> Este exemplo abreviado é mantido para explicar a adaptação de aplicações. Consulte [Docker Compose e x-casaos](./app-store-compose-x-casaos) para ver os campos v2 obrigatórios e as categorias permitidas.

```language
x-casaos:
    id: org.syncthing.syncthing       # ID estável em formato de domínio inverso
    architectures:                  # Lista de arquiteturas suportadas pela aplicação
        - amd64
        - arm
        - arm64
    version: "1.0.0"                # Versão apresentada pela loja v2
