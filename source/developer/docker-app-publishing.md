---
title: Build Apps
seo_title: "Build and Publish Docker Apps for the ZimaOS App Store"
...
> **Protocol scope:** This page focuses on adapting a single Docker app. For the current v2 store schema, locale key casing, repository layout, and generated output, use [Docker Compose and x-casaos](./app-store-compose-x-casaos), [Repository Structure](./app-store-repository-structure), and [Build Output](./app-store-build-output).
...
#### Legacy x-casaos Example

> This abbreviated example is kept for the app adaptation flow. Use [Docker Compose and x-casaos](./app-store-compose-x-casaos) for the complete v2 field requirements and allowed categories.

```language
x-casaos:
    id: org.syncthing.syncthing       # Stable reverse-domain app ID
    architectures:                  # List of architectures supported by the application
        - amd64
        - arm
        - arm64
    version: "1.0.0"                # App version exposed by the v2 store
