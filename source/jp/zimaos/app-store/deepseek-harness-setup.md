...
- **ローカル推論:** プロバイダーを自分のハードウェア上のモデルサーバーに向ければ、エージェントは完全にオフラインで動きます — **[ローカル LLM 推論](../local-llm-inference "検証済み35B MoE構成でZimaOS NAS上にプライベートAIを実行する")** をご覧ください。
- **dsh でサービスを自動化:** ZimaOS 上で自動化サービスを構築・運用します — **[ZimaOS でローカル AI エージェントを動かす](../local-ai-agent-on-zimaos "ZimaOS 上のローカル LLM サーバーに DeepSeek Harness を接続して最初の完全ローカルタスクを始める")** をご覧ください。
