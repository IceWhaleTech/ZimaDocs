...
AIモデル推論をローカルで加速するために使用されます（例えば、NVIDIAシリーズのグラフィックカード）。[GPU 拡張ガイド](../../hardware/gpu-expansion)を参照してください。
3. **容量6GB以上のハードドライブ**
...
Frigate を初めて使う場合は、[Frigate セットアップ](./frigate-setup "ZimaOS アプリストアから Frigate をインストールしてローカル AI カメラ監視を実現") のアプリストアからのワンステップインストールから始めてください。
