...
description: "Thunderbolt ケーブルで ZimaCube を Mac または PC に直接接続し、2GB/s の転送速度を実現します。ZimaOS の Thunderbolt PC ダイレクトモード設定ガイドです。"
type: "Docs"
...
## Mac
- まだ ZimaClient で ZimaCube に接続していない場合は、[ドキュメント](./features#Download-the-Zima-Client)を参照して [ZimaClient](https://www.zimaspace.com/zimaos/download) をダウンロードし、インストールしてください。ZimaClient は初回スキャン時に Thunderbolt 接続のデバイスを優先して検出します。
...
2. ケーブルを接続すると、ZimaClientは自動的に適応し、Thunderbolt接続に切り替えます。
   - まだ ZimaClient で ZimaCube に接続していない場合は、[ドキュメント](./features#Download-the-Zima-Client)を参照して [ZimaClient](https://www.zimaspace.com/zimaos/download) をダウンロードし、インストールしてください。
...
## 拡張読み物
  1. macOS の Finder と Windows のエクスプローラーで ZimaCube のファイルにアクセスして変更する方法については、[詳細](./samba-member-setup)を参照してください。
  2. ZimaCube の Thunderbolt 接続で実現できる最高転送速度については、[ZimaCube Thunderbolt 接続の転送速度分析](../hardware/transfer-speeds-over-thunderbolt)を参照してください。
