...
> **Alcance del protocolo:** Esta página se centra en adaptar una aplicación Docker individual. Para el esquema v2 actual, el uso de mayúsculas en las claves locale, la estructura del repositorio y los resultados de compilación, consulta [Docker Compose y x-casaos](./app-store-compose-x-casaos), [Estructura del repositorio](./app-store-repository-structure) y [Resultados de compilación](./app-store-build-output).
...
### Ejemplo heredado de x-casaos

> Este ejemplo abreviado se conserva para explicar la adaptación de aplicaciones. Consulta [Docker Compose y x-casaos](./app-store-compose-x-casaos) para conocer los campos v2 obligatorios y las categorías permitidas.

```language
x-casaos:
    id: org.syncthing.syncthing       # ID estable con formato de dominio inverso
    architectures:                  # Lista de arquitecturas soportadas por la aplicación
        - amd64
        - arm
        - arm64
    version: "1.0.0"                # Versión mostrada por la tienda v2
